...
namespace SwarmRobotics {
    ...
    void SwarmController::updateRobotState(int robot_id, double x, double y, double battery) {
    ...
    double SwarmController::calculateTaskCost(const Robot& robot, const Task& task) const {
        double distance = std::sqrt(std::pow(robot.x - task.x, 2) + std::pow(robot.y - task.y, 2));
        double battery_cost = (100.0 - robot.battery_level) / 10.0;
        return distance + battery_cost;
    }

    void SwarmController::allocateTasks() {
        decentralizedAllocation();
    }
    ...
    void SwarmController::broadcastAllocation(int task_id, int robot_id) {
        std::cout << "Task " << task_id << " allocated to Robot " << robot_id << std::endl;
    }
    ...
    CommunicationHandler::CommunicationHandler(boost::asio::io_context& io_context, int port)
        : socket(io_context, boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v4(), port)) {}

    void CommunicationHandler::sendMessage(const std::string& message, const std::string& ip, int port) {
